...
#Now I'm ready to start analyzing my 2006-2023 data. I want to plot the 'Total' Variable per year, using boxplots in ggplot2. I need to install ggplot2. 
library(ggplot2)
```

```{r gg_base, eval=TRUE, echo=TRUE, message=FALSE}
#I tell ggplot the data I will want to use. 
base=ggplot(data=YearsCombined)
```

```{r gg_geom_aes, eval=TRUE, echo=FALSE, message=FALSE}
#I add the geom. 
base + geom_boxplot(aes(x=Total, y=Year))
```

```{r}
#For the next exercise, I want only 2013 and 2023 data. I combine these two datasets
TwoYearsCombined <- rbind(YearEight_noNA, YearEighteen)
```

```{r}
#I confirm it appears as expected. 
str(TwoYearsCombined)
```

```{r}
#I check column names in TwoYearsCombined.
names(TwoYearsCombined)
```

```{r}
#I remove many of the columns I do not need
TwoYearsCombined_clean <- TwoYearsCombined[ , -c(8,9,10,11,12,13,14,15,16)]
```

```{r}
#I confirm my subset.
str(TwoYearsCombined_clean)
```

```{r facets_free_histo, eval=TRUE, echo=FALSE, message=FALSE}
#I apply faceting.  
base2=ggplot(data=TwoYearsCombined_clean)
base2 + geom_histogram(aes(x=Year), bins=10) +
  facet_wrap(vars~., scales = "Free", ncol=1)
```



In all plots, add all the text needed to explain the visual result.

On Github, publish the code and the output file. Publish the code using GitHub pages. The last week reinforced how to publish, it is not only create html on RStudio. Follow the steps shared on the previous session.